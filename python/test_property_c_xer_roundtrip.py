"""
Property-based test for C XER conversion round-trip.

Feature: asn1-spec-update, Property 3: C XER conversion round-trip

For any valid BER-encoded IpmstscdData message, converting to XER via the C
converter and back to BER should produce a structurally equal decoded message.
...
# Path to the C converter binary
C_CONVERTER = os.path.join(os.path.dirname(__file__), '..', 'c', 'converter-sample')


# --- Hypothesis strategies for generating valid IpmstscdData structures ---
# (Reused from test_property_ber_roundtrip.py, with float strategies constrained
# to values that survive the C converter's XER decimal text representation.
# The C converter formats REAL values with ~15 decimal digits of precision.
# Values with magnitude below ~1e-15 get truncated to 0.0 in XER output.
# We use floats with reasonable magnitude that represent realistic physical
# measurements and survive the XER text round-trip.)
...
def occupancy_based_detection_data_strategy(draw):
    ...
    val['occupancyRate'] = draw(st.integers(min_value=-100000, max_value=100000).map(lambda x: x / 100.0))
    if draw(st.booleans()):
        val['speed'] = draw(st.integers(min_value=0, max_value=30000).map(lambda x: x / 100.0))
...
def image_processing_based_detection_data_strategy(draw):
    ...
    if draw(st.booleans()):
        val['occupancyRate'] = draw(st.integers(min_value=0, max_value=10000).map(lambda x: x / 100.0))
    if draw(st.booleans()):
        val['speedQuantity'] = draw(st.integers(min_value=0, max_value=30000).map(lambda x: x / 100.0))
    if draw(st.booleans()):
...
def identification_based_detection_data_strategy(draw):
    ...
    if draw(st.booleans()):
        val['detectionSpeed'] = draw(st.integers(min_value=0, max_value=30000).map(lambda x: x / 100.0))
    if draw(st.booleans()):
...
# --- Helper functions for C converter ---

def convert_ber_to_xer(ber_bytes):
    """Convert BER bytes to XER using the C converter binary.

    Returns XER bytes on success, raises RuntimeError on failure.
    """
    with tempfile.NamedTemporaryFile(suffix='.ber', delete=False) as ber_file:
        ber_file.write(ber_bytes)
        ber_path = ber_file.name

    try:
        with tempfile.NamedTemporaryFile(suffix='.xer', delete=False) as xer_file:
            xer_path = xer_file.name

        result = subprocess.run(
            [C_CONVERTER, '-iber', '-oxer', ber_path],
            capture_output=True,
            timeout=30,
        )
        if result.returncode != 0:
            raise RuntimeError(
                f"C converter BER->XER failed (rc={result.returncode}): "
...
def convert_xer_to_ber(xer_bytes):
    """Convert XER bytes to BER using the C converter binary.

    Returns BER bytes on success, raises RuntimeError on failure.
    """
    with tempfile.NamedTemporaryFile(suffix='.xer', delete=False) as xer_file:
        xer_file.write(xer_bytes)
        xer_path = xer_file.name

    try:
        result = subprocess.run(
            [C_CONVERTER, '-ixer', '-oder', xer_path],
            capture_output=True,
            timeout=30,
        )
        if result.returncode != 0:
            raise RuntimeError(
                f"C converter XER->BER failed (rc={result.returncode}): "
                f"{result.stderr.decode('utf-8', errors='replace')}"
            )
        return result.stdout
    finally:
...
def test_c_xer_roundtrip(data):
    """
    **Validates: Requirements 7.2**

    Property 3: C XER conversion round-trip

    For any valid BER-encoded IpmstscdData message, converting to XER via the
    C converter and back to BER should produce a structurally equal decoded
    message.
    """
    # Step 1: Encode the generated data to BER using Python
    ber_bytes = ber_codec.encode(TYPE_NAME, data)

    # Step 2: Convert BER -> XER via C converter
    xer_bytes = convert_ber_to_xer(ber_bytes)

    # Step 3: Convert XER -> BER via C converter
    roundtrip_ber_bytes = convert_xer_to_ber(xer_bytes)

    # Step 4: Decode the round-tripped BER with Python
    decoded = ber_codec.decode(TYPE_NAME, roundtrip_ber_bytes)

    # Step 5: Assert structural equality
    assert decoded == data, (
        f"C XER round-trip failed.\n"
        f"Original: {data}\n"
        f"Decoded:  {decoded}\n"
        f"XER intermediate:\n{xer_bytes.decode('utf-8', errors='replace')[:1000]}"
    )


if __name__ == '__main__':
    test_c_xer_roundtrip()
